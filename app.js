...
const App = {
  ...
  async createDirectMessage(targetUserId) {
    ...
    const { data: room, error: roomErr } = await db
      .from('rooms')
      .insert({ type: 'direct', created_by: AppState.user.id })
      .select().single();

    if (roomErr) { UI.showToast('فشل إنشاء المحادثة', 'error'); return; }

    // Add both members
    await db.from('room_members').insert([
      { room_id: room.id, user_id: AppState.user.id, role: 'admin' },
  ...
  async createGroup(name, description, memberIds) {
    ...
    if (error) { UI.showToast('فشل إنشاء المجموعة', 'error'); return null; }

    // Add members (creator as admin + others)
    const members = [
      { room_id: room.id, user_id: AppState.user.id, role: 'admin' },
      ...memberIds.map(uid => ({ room_id: room.id, user_id: uid, role: 'member' }))
    ];
    await db.from('room_members').insert(members);

    // System message
    await db.from('messages').insert({
      room_id: room.id, sender_id: AppState.user.id,
